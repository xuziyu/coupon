...
import com.huey.dao.CouponDao;
import com.huey.entity.Coupon;
import com.huey.exception.CouponException;
import com.huey.feign.SettlementClient;
import com.huey.feign.TemplateClient;
...
public class UserServiceImpl implements IUserService {
    ...
    public List<Coupon> findCouponsByStatus(Long userId, Integer status) throws CouponException {
        ...
        //先去redis拿数据 没有去数据库
        List<Coupon> couponList=iRedisService.getCachedCoupons(userId,status);
        List<Coupon> preTarget;
        if(CollectionUtils.isNotEmpty(couponList)){
            log.debug("coupon cache is not empty:{},{}"+userId+status);
            preTarget=couponList;
        }else {
            List<Coupon> coupons= couponDao.findAllByUserIdAndStatus(userId,CouponStatus.of(status));
    ...
    public List<CouponTemplateSDK> findAvailableTemplate(Long userId) throws CouponException {
        ...
        // key 是 TemplateId
        // value 中的 left 是 Template limitation, right 是优惠券模板 配对(Pair)
